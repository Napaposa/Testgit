...
namespace ATD_ID4P.Class
{
    class Math3D
    {
        ...
        public class Cube
        {
            ...
            public Bitmap DrawCube(Point drawOrigin, Brush FontColor = null, Brush BackColor = null)
            {
                ...
                //message();
                FontColor = (FontColor == null ? Brushes.Peru : FontColor);
                BackColor = (BackColor == null ? Brushes.SaddleBrown : BackColor);
                for (int i = faces.Length - 1; i >= 0; i--) //draw faces from back to front
                {
                    switch (faces[i].CubeSide)
                    {
                        case Face.Side.Front:
                            if (fillFront)
                                g.FillPolygon(FontColor, GetFrontFace());
                            break;
                        case Face.Side.Back:
                            if (fillBack)
                                g.FillPolygon(BackColor, GetBackFace());
                            break;
                        case Face.Side.Left:
                            if (fillLeft)
                                g.FillPolygon(FontColor, GetLeftFace());
                            break;
                        case Face.Side.Right:
                            if (fillRight)
                                g.FillPolygon(BackColor, GetRightFace());
                            break;
                        case Face.Side.Top:
                            if (fillTop)
                                g.FillPolygon(FontColor, GetTopFace());
                            break;
                        case Face.Side.Bottom:
                            if (fillBottom)
                                g.FillPolygon(BackColor, GetBottomFace());
                            break;
                        default:
                            break;
                    }
